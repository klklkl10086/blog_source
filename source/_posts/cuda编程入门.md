...
- **缓存优化**：当一个warp（32个线程）中的所有线程都访问**同一个**常量内存地址时，硬件会将这个值从常量缓存广播到所有线程，**吞吐量接近一个标量指令**。如果warp内线程访问的是不同地址，这些访问会被**串行化**，导致性能急剧下降。
...
```cpp
// 常量内存：所有线程读取同一个 scale
__constant__ float scale;
float s = scale;   // warp 内统一地址 → 广播

// 共享内存：所有线程读取共享数组的同一元素
__shared__ float shared[128];
float val = shared[0];   // warp 内统一地址 → 广播（无 bank 冲突）
```
