...
- **核心机制**：每一步都**绝对服从**概率分布，直接挑选概率最高（$P_{max}$）的词。
- **多维评估**：
  - **确定性 (Deterministic)**：**高**。给定相同输入，输出永远一致。
  - **多样性 (Diversity)**：**低**。没有任何创造力可言。
  - **核心风险**：**Repetition, monotony (重复与单调)**。极易陷入局部最优解，导致模型在生成长文本时进入“死循环”（例如不断输出同一句话或词组）。
- **适用场景**：数学计算、代码生成、需要绝对忠实原文的机器翻译。

### 纯随机采样 (Random Sampling)

- **核心机制**：不对模型的输出分布做任何修改，完全按照原始概率进行“轮盘赌”式的随机抽样。
- **多维评估**：
  - **确定性 (Deterministic)**：**低**。
  - **多样性 (Diversity)**：**高**。
  - **核心风险**：**Incoherent tokens (语无伦次的词元)**。因为词表中包含大量概率极低的“长尾词”（错别字、生僻字、无关符号），纯随机采样无法屏蔽这些噪音，非常容易抽中它们，导致模型胡言乱语。
- **适用场景**：极少单独使用，通常作为理论对照。

### Top-K 采样 (固定数量截断)

- **核心机制**：人为设定一个固定值 $K$。模型强制忽略排名在 $K$ 之后的词，仅在概率排名前 $K$ 的词中重新分配概率并采样。
- **多维评估**：
  - **确定性 (Deterministic)**：**中**。
  - **多样性 (Diversity)**：**中**。
  - **核心风险**：**Fixed K mismatches distribution shape (固定的 K 值无法适配动态的分布形状)**。
    - 当模型**很确信**时（如前两个词占了 99% 的概率），$K=50$ 会强行塞入 48 个垃圾词汇增加出错率。
    - 当模型**很发散**时（如 100 个词都很合理），$K=50$ 又残忍切断了本该有的多样性。
- **适用场景**：过去常用，现在多作为一种粗粒度的“前置安全网”（例如先过一遍 $K=100$，再进行其他采样）。

### Top-P 采样 (核采样 Nucleus Sampling)

- **核心机制**：不固定词的数量，而是固定**概率总和**（阈值 $P$）。将词按概率降序排列并依次累加，直到累加和达到 $P$（例如 $P=0.9$），只在这个动态大小的核心词集中采样。
- **多维评估**：
  - **确定性 (Deterministic)**：**中**。
  - **多样性 (Diversity)**：**中**。
  - **核心风险**：无明显短板，**Well-balanced in practice (在实际应用中最均衡)**。它完美解决了 Top-K 的痛点——模型确信时，候选池自动缩小；模型发散时，候选池自动放大。
- **适用场景**：目前工业界的**绝对主力策略**。广泛用于对话、闲聊、创意写作。

### 束搜索 (Beam Search)

- **核心机制**：放弃“走一步看一步”，而是维护 $B$ 条（Beam Size，通常为 3-5）全局概率最高的“探索路径”。每步都基于这几条路径往前推演，最后选择一条全局累加概率最大的完整句子。
- **多维评估**：
  - **确定性 (Deterministic)**：**高**。
  - **多样性 (Diversity)**：**低**。
  - **核心风险**：**Expensive, safe but dull (开销大，安全但乏味)**。因为要同时计算和存储多条路径，算力成本远高于贪心搜索。同时，它极度追求全局最高分，导致生成的文本往往是训练集中最四平八稳的“套话”，极其缺乏人类表达的生动性。
- **适用场景**：对算力不敏感且需要极高准确率的短文本任务（如专业文本翻译）
...
**低温状态 ( $0 < T < 1$，例如 0.2)**：**放大差距**

- **机制**：由于 $T$ 是小数，除以 $T$ 相当于将所有的 Logits 放大。经过指数运算后，原本得分稍微高一点的词，其概率会被无限放大，迅速逼近 100%；而其他词的概率会被极度压缩，趋近于 0。
- **表现**：模型变得非常自信、死板。
- **极限情况**：当 $T \to 0$ 时，模型 100% 只选得分最高的词，这在数学上完全等效于**贪心搜索 (Greedy Search)**。

**常温状态 ( $T = 1.0$)**：

- **机制**：除以 1 不改变原始数值。
- **表现**：完全真实反映模型在预训练时学到的自然概率分布。

**高温状态 ( $T > 1$，例如 1.5 或 2.0)**：

- **机制**：由于 $T$ 大于 1，Logits 的数值会被缩小，最高分和最低分之间的差距被抹平。
- **表现**：原本概率很低的“冷门词”获得了极大的曝光机会，最高概率词的优势被削弱。生成的文本跳跃性强，甚至会出现生造词或胡言乱语。
- **极限情况**：当 $T \to \infty$ 时，所有词的概率变得完全一样（均匀分布），模型处于彻底的随机盲猜状态。

Temperature常常和Top-P结合使用：**先用 Temperature 把分布拉平（增加创造力），再用 Top-P 切掉长尾的垃圾词**
