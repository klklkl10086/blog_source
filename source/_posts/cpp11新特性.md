...
`nullptr` 是 C++11 引入的一个**关键字**，代表一个**空指针字面量**（null pointer literal）。它的类型是 `std::nullptr_t`（定义在 `<cstddef>` 中），可以隐式转换为任意指针类型或成员指针类型，但**不能转换为整数类型**。
...
### 限制（C++11）：
- 函数体只能包含一条 `return` 语句（不能有局部变量、循环、`if` 等，但可以用 `? :` 和递归）。
- 参数和返回值类型必须是**字面类型**（LiteralType，如标量、引用、字面类等）。
- 函数体不能有 `try` 块或 `asm` 声明。

```cpp
constexpr int square(int x) {
    return x * x;          // 一条 return 语句
}
...
用户自定义类型可以通过 `constexpr` 构造函数创建编译期对象。该类必须满足**字面类型**要求：
- 所有数据成员都是字面类型。
...
int main() {
    constexpr int g = gcd(36, 24);   // 编译期计算 12
    std::cout << g << std::endl;
