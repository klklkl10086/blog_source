...
```c
#include<arpa/inet.h>
in_addr_t inet_addr(const char* strptr);//点分十进制IPv4字符串转换为网络字节序的整数IP地址 失败返回INADDR_NONE
int inet_aton(const char* cp,struct int_addr* inp);//作用同上，但是是将cp字符串转换为IP存入inp中，成功返回1失败返回0
char* inet_ntoa(struct in_addr in);
//网络字节序整数的IPv4地址转换为点分十进制的IP地址字符串，由于使用静态变量存储转换结果，函数的返回值指向该静态内存因此该函数不可重入

```
使用以下函数也可以进行转换并且同时适用于IPv4和IPv6：
```c
#include <arpa/inet.h>
int inet_pton(int af,const char* src,void* dst);
//src的点分十进制（IPv4或者IPv6）转换为网络字节序的IP地址，将结果存储到dst指向的内存里，af参数指定地址族 AF_INET  AF_INET6，成功返回1失败返回0并设置errno
const char* inet_ntop(int af,const void* src,char* dst,socklen_t cnt);
//执行相反的操作，前三个参数和inet_pton一样，cnt指定目标存储单元大小，成功返回目标存储单元的地址，失败返回NULL并设置errno
```
...
## IPv6专用地址结构 'socketaddr_in6'
```c
struct sockaddr_in6
{
	sa_family_t sin6_family;//地址族
	u_int16_t sin6_port;//端口号 网络字节序表示 htons转换
	u_int32_t sin6_flowinfo;//流信息 应该设置为0
	struct in6_addr sin6_addr;//IPv6地址结构体
	u_int32_t sin6_scope_id;//scope ID
};
struct in6_addr
{
	unsigned char sa_addr[16];//IPv6地址，网络字节序表示htonl转换
};
```
> 所有专用socket地址类型变量在实际使用的时候都要转换为通用socket地址类型sockaddr（强制转换即可）因为所有socket编程接口使用的地址参数类型都是`sockaddr`
...
- 返回的字符串指向静态缓冲区，后续调用会覆盖，因此不是线程安全的。如需保留，应拷贝出来（不可重入）。
